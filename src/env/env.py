...
#         ], # 17 spaces
#         pricing_columns = ['CO2_ele','CO2_gas', 'Ele_buy', 'Ele_sell', 'Gas_buy' ,'Dem_cha']
        
#     ):
        
#         self.model_folder = model_folder
#         self.demand_data = demand_data
#         self.pricing_columns = pricing_columns
...
#         # energy cost 
#         buy_price = demand_data['Ele_buy'] # $/kwh
#         sell_price = demand_data['Ele_sell'] # $/kwh
#         dem_price = demand_data['Dem_cha'] # $/kwh
#         gas_price = demand_data['Gas_buy'] # $/kg

#         # Carbon emissions
#         CO2_ele = demand_data['CO2_ele']*1000 # kg co2/kW
#         CO2_gas = demand_data['CO2_gas']*1000 # kg co2/kg fuel

       
#         # outputs:
#         p_grid = electrical_output['electricalGroup.Pgri.real']/1000 # in kWh
#         m_gas = sum(boiler_output['heatingGroupFmu.mFueChp']+ boiler_output['heatingGroupFmu.mFueBoi'])*3600 #in kg/h
#         p_norm = (boiler_output['heatingGroupFmu.preChp.p'] + boiler_output['heatingGroupFmu.preBoi.p'])/(2*9*10**5) # pa/pa pressure setpoint 
#         t_norm = (chiller_output['chillersFMU.TchiAbsSup'] + chiller_output['chillersFMU.TchiSup'])/(2*280.15) # K/K temp setpoint
...
#         Peak_dem = max(p_grid[:720])
#         Cdem =  Peak_dem * dem_price
...
#     price_data = pd.read_csv('../../EmissionsPrice.csv').iloc[:price_data.shape[0]]
